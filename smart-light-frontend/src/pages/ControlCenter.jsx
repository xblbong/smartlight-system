...
export default function ControlCenter({ token, onUnauthorized }) {
  ...
  const handleControl = async (deviceId, zone, action, silent = false, skipRefetch = false) => {
    ...
    try {
      ...
      if (!silent) {
        addToast(
          `✓ Perintah ${action} dikirim ke Zone ${zone} (${deviceId})`,
          "success",
        );
      }
      ...
      if (!silent) {
        addToast(
          getErrorMessage(error, `Gagal mengirim perintah ${action}.`),
          "error",
        );
      }
      return false;
    ...
    <div>
      ...
      <div className="page-header" style={{ marginBottom: "32px" }}>
        <div>
          ...
          >
            Kontrol langsung sistem pencahayaan adaptif. Gunakan untuk
            pemeliharaan, keadaan darurat, atau acara kampus.
          </p>
        </div>
        <button
      ...
      <div className="control-layout">
            ...
            devices.map((dev, idx) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div
                      className={`ctrl-zone-icon ${offline ? "off" : isOn ? "on" : "off"}`}
                    >
                      <MapPin size={20} />
                    </div>
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        {dev.zone_name || `Zone ${dev.zone}`} — {dev.device_id}
                      </div>
                      <div
                        style={{
                          display: "flex",
                      ...
                      >
                        {offline ? (
                          <span
                            className="control-status-chip off"
                          ...
                          <span
                            className={`control-status-chip ${isOn ? "on" : "off"}`}
                          >
                            <Lightbulb size={11} />{" "}
                            {d.kondisi || (isOn ? "NYALA" : "MATI")}
                          </span>
                        )}
                        {isFaulty && (
                        ...
                        {d.kondisi?.includes("DARURAT") && (
                          <span
                            className="control-status-chip"
                            style={{
                              background: "#dc2626",
                              color: "white",
                        ...
                        >
                          PWM: {d.powerLampu} | {d.voltage} V | {d.current} mA | Lux:{" "}
                          {d.lux} lx
                        </span>
                      </div>
                    </div>
                  ...
                  >
                    ...
                    >
                      {isOn ? dev.lamp_count || 2 : 0} / {dev.lamp_count || 2}{" "}
                      lampu aktif
                    </span>
                  ...
                  <div className="control-btns">
                    <button
                      ...
                      disabled={isPend}
                      onClick={() =>
                        handleControl(dev.device_id, dev.zone, "ON")
                      }
                      style={{ minWidth: "100px" }}
                    >
                      {isPend && pending[key] === "ON" ? (
                        <>
                          <div className="spinner-sm" /> Mengirim...
                        </>
                      ) : (
                        "NYALAKAN"
                      )}
                    ...
                    <button
                      ...
                      disabled={isPend}
                      onClick={() =>
                        handleControl(dev.device_id, dev.zone, "OFF")
                      }
                      style={{
                        minWidth: "100px",
                        background: "#f3f4f6",
                    ...
                    <button
                      ...
                      disabled={isPend}
                      onClick={() =>
                        handleControl(dev.device_id, dev.zone, "AUTO")
                      }
                      style={{
                        minWidth: "100px",
                        background: "#e0f2fe",
                    ...
                    >
                      {isPend && pending[key] === "EMERGENCY" ? (
                        <>
                          <div className="spinner-sm" /> Mengirim...
                        </>
                      ) : (
                        <>
                          <Siren size={13} /> DARURAT
                        </>
                      )}
                    </button>
                  </div>
        ...
        <div className="control-sidebar">
          ...
          >
            ...
            >
              Nyalakan atau matikan semua lampu serentak. Gunakan saat situasi
              darurat atau pemeliharaan.
            </p>
            {/* <button
              className="btn"
          ...
          <div className="card">
            ...
            <div className="zc-progress-bg">
              <div
                ...
                style={{
                  width: `${devices.length > 0 ? (onCount / devices.length) * 100 : 0}%`,
                  background: "var(--sidebar-bg)",
                }}
              />
            </div>
            ...
            >
              {devices.length > 0
                ? Math.round((onCount / devices.length) * 100)
                : 0}
              % zona aktif
            </div>
          </div>
