...
export default function ThresholdSettings({ token, onUnauthorized }) {
  const [settings, setSettings] = useState({ ldr_sensitivity: 420, pir_delay: 3 })
  ...
  const handleReset = () => {
    setSettings({ ldr_sensitivity: 420, pir_delay: 3 })
    setRawInput('420')
    setInputError('')
  }

  // Menghapus kalkulasi fiktif effPct atas permintaan dosen
        ...
        <div className="card" style={{ background: 'linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%)', color: 'white', padding: '40px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <div style={{ marginTop: '32px', padding: '16px', background: 'rgba(255,255,255,0.08)', borderRadius: '10px' }}>
            ...
            <div style={{ fontSize: '13px', color: 'white' }}>
              Lampu diprioritaskan menyala jika mendeteksi objek <strong>(jarak &lt; 30 cm)</strong> dan intensitas lingkungan <strong>&lt; {settings.ldr_sensitivity} lux</strong>
            </div>
          </div>
        </div>
      </div>
